...
Therefore a cone has $V=\frac13\pi r^2h$. The height is perpendicular to the base. A pyramid edge or cone slant height cannot replace it unless the problem establishes that they are equal.
...
For a ball of diameter $12$ cm, the radius is $6$ cm, so $V=\frac43\pi\times6^3=288\pi\text{ cm}^3$. Always divide a given diameter by $2$ before using a radius formula.
