...
A linear function has form $f(x)=ax$. Its table is proportional, its graph is a straight line through the origin, and $a$ is its proportionality coefficient and slope. If a straight-line graph does not pass through the origin, the relationship is not proportional.

## Affine Functions

An affine function has form $f(x)=ax+b$. Its graph is a straight line. The number $a$ is the slope: when $x$ increases by $1$, $f(x)$ changes by $a$. The number $b=f(0)$ is the vertical intercept.

- If $a>0$, the function increases.
- If $a<0$, the function decreases.
- If $a=0$, the function is constant.

From two points with distinct inputs, $a=\frac{y_2-y_1}{x_2-x_1}$. Then substitute one point into $y=ax+b$ to find $b$.

## Worked Reconstruction

A line through $(1,5)$ and $(4,11)$ has slope $a=\frac{11-5}{4-1}=2$. Using $(1,5)$ gives $5=2\times1+b$, so $b=3$. The function is $f(x)=2x+3$. Check: $2\times4+3=11$.

## Modeling a Situation

A tank containing $120$ liters and losing $8$ liters per minute is modeled by $V(t)=120-8t$. The intercept is the initial volume; the slope is the rate of change. The model is meaningful only for $0\le t\le15$.

Always define variables, include units, and state the realistic domain.

## Method Choice and Misconceptions

- Use substitution for an image and an equation for a preimage.
- Use two points to find a slope; do not divide $y$ by $x$ unless the line passes through the origin.
- Keep the subtraction order consistent in the slope formula.
- Graphical readings are often approximate, while a formula can give an exact result.
- The vertical intercept is $b$, not the point $(0,b)$ itself.

## Extension: Comparing Two Models

Solve $ax+b=cx+d$. If $a\ne c$, there is one intersection. If $a=c$ and $b\ne d$, the lines are parallel. If both coefficients match, the formulas define the same line.
