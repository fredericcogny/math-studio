...
exercises:
  ...
  - id: 2de-trig-t3
    ...
    solution: >-
      Since $(AH)$ is perpendicular to $d$ and $M$ belongs to $d$, the triangle $AHM$ is right-angled at $H$. The Pythagorean theorem gives $AM^{2}=AH^{2}+HM^{2}=36+64=100$, so $AM=10$. For any point $M$ of $d$ other than $H$, the same triangle is right-angled at $H$, so $AM^{2}=AH^{2}+HM^{2}>AH^{2}$ because $HM>0$, hence $AM>AH$. The projection $H$ is therefore the closest point, and $AH$ is by definition the distance from $A$ to $d$. The area of $AHM$ is $\dfrac{AH\times HM}{2}=\dfrac{6\times 8}{2}=24$.
    hints:
